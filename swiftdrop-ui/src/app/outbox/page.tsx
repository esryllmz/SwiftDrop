...
export default function OutboxPage() {
  ...
  return (
    <div>
      <PageHeader
        title="Outbox"
        description="Event Stream and Kafka publishing monitor for logistics events."
                ...
                <tbody className="divide-y divide-slate-800 bg-slate-950">
                  {events.map((event) => (
                    <tr key={event.id} className="align-top">
                      <td className="px-3 py-2 text-slate-300" title={event.id}>
                        {shortId(event.id)}
                      </td>
          ...
          {selectedEvent ? (
            <div className="grid gap-4">
              <dl className="grid gap-3 text-sm">
                ...
                <DetailRow
                  label="aggregateType"
                  value={selectedEvent.aggregateType}
                />
                <DetailRow
                  label="aggregateId"
                  value={selectedEvent.aggregateId}
                />
                ...
                <DetailRow
                  label="retryCount"
                  value={String(selectedEvent.retryCount)}
                />
                <DetailRow
                  label="lastError"
                  value={selectedEvent.lastError ?? "-"}
                />
                <DetailRow
                  label="createdAt"
                ...
                <DetailRow
                  label="sentAt"
                  value={formatDateTime(selectedEvent.sentAt)}
                />
                <DetailRow
                  label="correlationId"
